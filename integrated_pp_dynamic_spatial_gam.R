model{
	...
	for(g in 1:G){
		...
		log(lambda[g, year]) <- inprod(X[g,], b) +
			inprod(occ_covs[g,1:nlatent ], beta_occ[1:nlatent]) +
			cell_area + occ_season[year]
  ...
  # slope terms
	...
	occ_tau_season ~ dgamma(1,1)
	occ_sd_season <- 1 / sqrt(occ_tau_season)
	occ_season[1] ~ dnorm(0, 0.75)
	  for(yr in 2:nyear){
		  occ_season[yr] ~ dnorm(occ_season[yr-1], occ_tau_season)
	  }
	...
	pa_season[1] ~ dnorm(0, 0.75)
	for(yr in 2:nyear){
		pa_season[yr] ~ dnorm(pa_season[yr-1], pa_tau_season)
	}
	...
	po_season[1] ~ dnorm(0, 0.75)
	for(yr in 2:nyear){
		po_season[yr] ~ dnorm(po_season[yr-1], po_tau_season)
	}
