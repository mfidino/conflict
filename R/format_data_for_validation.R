...
# drop the third month
year_month_val <- year_month[seq(3, nrow(year_month), by = 3),]
year_month <- year_month[-seq(3, nrow(year_month), by = 3),]


# combine that with presence only data

# get just podat for validation
podat_val <- dplyr::inner_join(
	podat,
	year_month_val,
	by = c("year", "month")
)
